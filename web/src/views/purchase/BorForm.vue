    ...
    >
      <transition name="el-fade-in">
        <template v-if="bill.dialog">
          <Bor :source="bill.source" @destroy="bill.dialog = false"></Bor>
        </template>
      </transition>
    </el-dialog>
    <el-dialog
...
export default {
  ...
  methods: {
    ...
    downTemplate() {
      ...
      setTimeout(() => {
        window.open(
          this.store.base.cdnSite + "/erp/batch/采购订单导入模板.xlsx"
        );
      }, 1000);
    },
    //上传模板回调
    ...
    exports(scene) {
      if (this.tableSelection.length == 0) {
        ...
        setTimeout(() => {
          window.open(this.$base.web + "bor/exports?" + parm);
        }, 1000);
      }
    },
    //关联订单页面切换
