<template>
  <div class="sellForm area">
    <div class="layout">
      ...
      <el-button-group>
        <template v-if="tableSelection.length != 0">
          ...
          <template v-if="$lib.getUserRoot(store.root, 'sell', 'del')">
            <el-button @click="del(tableSelection)" type="info">删除</el-button>
          </template>
        </template>
        <template v-if="$lib.getUserRoot(store.root, 'sell', 'batch')">
    ...
    >
      <transition name="el-fade-in">
        <template v-if="bill.dialog">
          <Sell :source="bill.source" @destroy="bill.dialog = false"></Sell>
        </template>
      </transition>
    </el-dialog>
    <el-dialog
    ...
    >
      <transition name="el-fade-in">
        <template v-if="bills.dialog">
          <Sor :source="bills.source" @destroy="bills.dialog = false"></Sor>
        </template>
      </transition>
    </el-dialog>
    <el-dialog
    ...
    >
      <transition name="el-fade-in">
        <template v-if="bills.dialog">
          <Sre :source="bills.source" @destroy="bills.dialog = false"></Sre>
        </template>
      </transition>
    </el-dialog>
    <el-dialog
...
import Sre from "@/views/sale/Sre";
...
export default {
  ...
  methods: {
    ...
    record(page) {
      ...
      this.$axios.post("sell/record", parm).then((result) => {
    ...
    downTemplate() {
      ...
      setTimeout(() => {
        window.open(this.store.base.cdnSite + "/erp/batch/销售单导入模板.xlsx");
      }, 1000);
    },
    //上传模板回调
    ...
    exports(scene) {
      if (this.tableSelection.length == 0) {
        ...
        setTimeout(() => {
          window.open(this.$base.web + "sell/exports?" + parm);
        }, 1000);
      }
    },
    //关联订单页面切换
