      ...
      <el-button-group>
        <template v-if="tableSelection.length != 0">
          ...
          >
            ...
            <ul>
              <template v-if="$lib.getUserRoot(store.root, 'sre', 'examine')">
          ...
          <template v-if="$lib.getUserRoot(store.root, 'sre', 'del')">
            <el-button @click="del(tableSelection)" type="info">删除</el-button>
          </template>
        </template>
        <template v-if="$lib.getUserRoot(store.root, 'sre', 'batch')">
...
export default {
  ...
  methods: {
    ...
    downTemplate() {
      ...
      setTimeout(() => {
        window.open(
          this.store.base.cdnSite + "/erp/batch/销售退货单导入模板.xlsx"
        );
      }, 1000);
    },
    //上传模板回调
    ...
    exports(scene) {
      if (this.tableSelection.length == 0) {
        ...
        setTimeout(() => {
          window.open(this.$base.web + "sre/exports?" + parm);
        }, 1000);
      }
    },
    //关联订单页面切换
